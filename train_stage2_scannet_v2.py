"""
Stage 2 v2: soft weighted merge 적용
- soft_merge_with_scores로 L_recon → TokenScorer gradient 직접 흐름
- L_kl_aux 제거 (gradient 우회 불필요)
- Loss = L_recon + lambda_ratio * L_ratio
"""
import os, sys, math
import numpy as np
import torch
...
def main():
    ...
    p.add_argument('--output_dir', default='./checkpoints/stage2_scannet_v2')
    ...
    for step in range(args.total_steps):
        ...
        # L_recon — soft merge로 TokenScorer까지 gradient 직접 흐름
        ...
        # ── L_kl_aux 제거 (soft merge로 gradient 직접 흐름) ──────────────
        ...
        #     with torch.no_grad():
        #         ga = compute_info_maps(imgs_n, patch_tokens_cached.detach())["info_map"]
        #     ga_flat   = ga.float().reshape(B*S, -1)
        #     pred_s    = model.aggregator.token_scorer(patch_tokens_cached.detach(), Hp=Hp, Wp=Wp)
        ...
        L_total = L_recon + args.lambda_ratio * L_ratio
        # L_kl 제거 — soft merge로 L_recon이 TokenScorer까지 직접 역전파
        ...
        if step % args.save_every == 0 and step > 0:
            path = os.path.join(args.output_dir, f"stage2v2_step{step}.pt")
            torch.save({
                'step': step,
    ...
    path = os.path.join(args.output_dir, "stage2v2_scannet_final.pt")
...
# ── 서버 실행 명령어 ─────────────────────────────────────────────────────
# python train_stage2_scannet_v2.py \
#   --stage1_checkpoint ./checkpoints/stage1/scorer_stage1_epoch5.pt \
#   --vggt_checkpoint   te_dict.pt \
#   --scene_dir         ./litevggt_dataset/scannet/scene0025_01 \
#   --output_dir        ./checkpoints/stage2_scannet_v2 \
#   --total_steps       10000 \
#   --batch_size        1 \
#   --num_frames        8 \
#   --frame_skip        10 \
