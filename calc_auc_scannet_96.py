...
if __name__ == "__main__":
    color_96_dir = './litevggt_dataset/scannet/scene0025_01/color_96'
    pose_dir     = './litevggt_dataset/scannet/scene0025_01/pose'
    frame_names  = sorted(os.listdir(color_96_dir))
    print(f"평가 프레임 수: {len(frame_names)}")
    gt_poses = load_gt_poses(pose_dir, frame_names)
    print(f"GT pose 로드 수: {len(gt_poses)}")
    b_auc = eval_poses('./results/baseline_scannet_96/pred_poses.npy', gt_poses, "Baseline (GA map)")
    p_auc = eval_poses('./results/proposed_scannet_96/pred_poses.npy', gt_poses, "Proposed (TokenScorer)")
    print(f"\n{'='*40}")
    print("비교 요약")
    print(f"{'='*40}")
    for k in b_auc:
